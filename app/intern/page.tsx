...
import { Calendar, Clock, Sparkles, CheckSquare, Package } from 'lucide-react';
    ...
    <div className="space-y-8 pb-12">
      <div className="relative overflow-hidden bg-gradient-to-r from-[#4FBACA] via-[#3AA8BC] to-[#2A9FB4] rounded-2xl p-8 shadow-2xl">
        ...
        <div className="relative z-10">
          <h2 className="text-4xl font-bold text-white drop-shadow-lg">{t('volunteer_dashboard')}</h2>
          <p className="text-xl text-white/90 font-medium mt-1">{t('view_manage_attendance')}</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Button
          onClick={() => router.push('/intern/sessions')}
        ...
        >
          <div className="flex flex-col items-center gap-2">
            ...
            <span className="text-lg font-semibold">{t('manage_attendance')}</span>
          </div>
        </Button>
            ...
            <div className="space-y-4">
              {todaySessions.map((session, index) => (
                <div
                  ...
                  onClick={() => router.push(`/intern/sessions/${session.id}`)}
                >
